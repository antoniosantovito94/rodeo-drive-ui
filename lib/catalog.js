...
async function getPublishedProducts({
  ...
  if (!supabase) {
    return filterFallbackProducts({ featuredOnly, newArrivalOnly, categorySlug, productType });
  }
...
function normalizeProduct(product) {
  ...
  const badge = product.badge_label ?? (isNewArrival ? "Nuovo" : null);
  ...
  return {
    ...
    badge,
    badgeClass: getBadgeClass(product.badge_tone ?? (badge === "Nuovo" ? "blue" : "")),
